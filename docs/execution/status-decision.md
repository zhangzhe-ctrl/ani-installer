...
- CI 首跑实测（run 36143751358，推送后 2m16s）：红。唯一失败是 R02 对照子测试 `TestVerifyNeverRepairsNetwork/control`——它从 `git show HEAD:` 取 pre-R02 旧脚本，本地跑在修复入册前的 HEAD 上恒绿，一旦收口提交合入（CI 的 HEAD 已含修复）必然不成立，属"时间炸弹"式测试缺陷，由本次 CI 首跑如期暴露（这正是 R04 补 CI 的价值）。修复：对照钉死在 blueprint 提交 `275827f`（仍缺历史时 Skip 而非误判红），CI checkout 改 `fetch-depth: 0`；修复与本补记同提交推送，首跑转绿后 R04 的 ci_not_run 才算闭合。
- CI 闭环（run 36144627406，修复提交 a6c09ff 推送后 2m23s）：**成功**。首跑还牵出另外两枚同族时间炸弹（test-lab-credentials CTRL-R01a/b、test-ani-task-errors T-R03-01e/02c 的 `git show HEAD:` 对照），随 a6c09ff 一并钉死；三处钉死均为 blueprint 提交 275827f，浅克隆/缺史环境下 Go 侧 Skip、python 侧显式失败，不会静默放行。R04 状态改为 code_fixed_local_gate_pass_ci_green。首跑失败日志归档 ci/run1-36143751358-failure.log（仓外证据目录）。
...
## R 整改轮 F01–F12｜代码阶段收口（2026-09-25 23:00 → 2026-09-26，MODE=code，无实机）
- 任务包 `docs/execution/remediation/20260925/`（goal/spec/audit + 隔离复现证据 zip，SHA256SUMS 5/5 校验通过），基线 HEAD `f58430e`。本轮只做代码整改+隔离回归+同源构建：未执行真实集群安装、Pod 删除、快照还原、节点重启、清缓存，也未提交/推送（工作树 50 项改动全部保留在 `main` 之上）。
- 六组连续推进（G3→G1→G2→G4→G5→G6），每组完成后跑统一门禁并记树指纹：G3 `422aa916`、G1 `c6c76138`/`e03c610b`、G2 `5e810a83`→`5b16c585`、G4 `6015fe32`、G5 `c3310821`、G6（扩展门禁）`bb326651`，逐组 rc 均为 0；日志与 WORKLOG 在 `ani-installer-work/evidence/F-remediation-20260925/`。
- 43 例最小回归矩阵落位：T01–T11（G1 真实分层/一次性账本/取消/真实数据）、T12–T21（G2 现场身份/所有权/共享锁/执行主机/计划篡改/连续新增）、T22–T24 与 T41–T42（G3 身份合同与真实预检、G2 组件有效 SC、G6 blkid 三分）、T25–T32（G4 材料精确绑定/ISO 内容/store 门）、T33–T36 与 T37–T40、T43（G5 render 语义与 F12 默认路径、G6 门禁触发面与冷启动证据）。负向一律按“必须命中缺陷”写断言，无删检查、无放宽、无吞错、无把本轮缺陷重标 out-of-scope。
- 上一轮“范围外登记”四项本轮已按缺陷闭合：①成功轮 run-state phase 停留 `registry_content_verified` → 结果/阶段分离并记最后完成阶段；②`sourceTreeFingerprint==configDigest` → 六项真实来源身份字段分离（构建期 ldflags 注入树指纹，实测二进制内嵌）；③components execute `--kk` 的 PATH 假设 → 默认 `os.Executable()` 且与计划绑定的二进制摘要一致；④缺陷8（metrics verify 的 `PY_EOF` heredoc 不在 R08 豁免而 render 非零）→ heredoc 三分类后 render 全选择通过。
- 签发边界与未闭合（精确，不止“环境问题”）：F01–F12 一律 `code_fixed + local_gate_pass`；**ci_pass 不写**（本轮禁止提交/推送，CI 未被触发）；`live_pass_selected` 不写——选定组合复验（干净首装→产品自动成功记录→smoke→计划新增 valkey→同版本只读 no-op→授权专项一次）需单独授权，且新增了两项必须实机确认的收紧：首装对**每一行** images.tsv 摘要核对（含 kubeovn 行是否仍走原 kcn 表的路径）、真 `helm template` 展开离线 Chart 与真 hauler store 门。无法本地完成的项目在 WORKLOG「未闭合」小节列出下一条可执行动作。
- 收口实测（08:4x）：冻结源码后统一门禁 **rc=0**，树 `63737c9f…`（`evidence/F-remediation-20260925/final-gate-run2.log`）；`scripts/build-code.sh` **rc=0** 产出同源 code release `kubekey/build/ani-code-20260926-084116`（gate/build/package 三处树指纹一致，SHA256SUMS 5/5，kk `b29628db…`，二进制内嵌该指纹）。中途一次真实反证：门禁运行期间改动 `kubekey/docs/progress.md` 即被守卫拒绝构建（before `bb326651` ≠ after `9d8957d5`），冻结后重建才通过——发布身份绑定不是纸面承诺。
- 发布物本身的行为证据（不依赖 fixture）：在无 builtin 源码树、无 helm 的目录用该 kk 渲染 base 站点得 16 个文件（kcn/envoy/smoke 全在，证 T33 选择在真二进制成立），且无 chart 组件时输出“nothing to do”而非错误索要 Helm。
- 逐条状态、不变边界、未闭合缺口与下一条动作：见 `docs/execution/progress.yaml` 的 F01–F12 条目与 `evidence/F-remediation-20260925/task-result.yaml`（含 WORKLOG 分节）。

## F-live｜选定组合实机复验与 L-04/L-05/L-06 定点收尾（2026-09-25 23:26 → 2026-09-26 22:36，MODE=live，父任务 F-LIVE）
- 上一节（F01–F12 代码阶段收口）写的签发边界在实机轮之后必须改写而非删除：**`live_pass_selected` 本轮已实测成立并写入 `progress.yaml` 的 F-LIVE**（干净首装→产品自动成功记录→底座 smoke→计划新增 valkey→同版本只读 no-op→`ani verify` 消费新增记录），原“不写”条款只适用于那一节所描述的 code 阶段。代码阶段新增的两项收紧（首装逐行核对 images.tsv 摘要、真 `helm template` 展开离线 Chart 过真 hauler store 门）也在同一底座上实机验证过，不再挂“待授权”。
- 实机暴露的三个缺陷按“先取证→回归先红→定点修→再实机”的顺序闭合，都不放宽检查：L-04（Envoy 一次性请求撞节点 kube-proxy 编程窗口，探针改为有界重试、每次新建 Pod、两标记必须各自 stdout 命中）、L-05（ownership 标签只落 Service 使安装器认不出自己的 StatefulSet、同版本 no-op 不可达，只改两处模板）、L-06（新增组件后 `ani verify` 无从消费，落地 `components-execution` 记录并显式分派）。L-05 同时纠正 R16 对同一现象（postgresql 重复规划 rc=1）的错误归因。
- 复核轮对 L-06 的自我结论做了独立审计（6 路只读 + 每条 blocker 两票对抗驳回，4 成立 / 2 驳回），确认并修掉三条实现与声明不符：①**记录类型只靠 `recordKind` 字符串分派**——把落地记录改写为 `install-success` 就绕过 base 记录字节、证据摘要、实时集群 uid 全部绑定校验，并把一次性 Pod 删除配额键改成记录自身的子 run（等于观察记录也能重开额度）；现按 `componentsExecution` 块是否存在做结构化判定，消费侧与写侧 `--base-run` 选择同时拒绝。②**playbook 成功但连接事实聚合失败**这条已经改动集群的路径完全不落记录，正是 L-06 原症状复发；现与 playbook 失败一样落 `result=failed` 记录，且记录写入失败不再被静默丢弃。③**metrics 的 WorkloadName 写成 `ani-metrics-prometheus`，角色实际创建 `prometheus-ani-metrics-prometheus`**（`kubectl get sts -A` 实机直读为证），uid 读不到即目标降级 `unknown`，轻则记录被自身消费者拒绝、重则装完后 execute 非零退出且无记录。
- 驳回的两条（保留结论与理由，避免下轮重复提出）：①“`status-decision.md` 缺 F-live 小节所以会让读者停在整改前状态”——文件事实成立（该文件此前确无 L-04/05/06 任何字样），但“读者会被误导”的因果被驳回：本节即是补上的裁决位，且逐条状态一直写在 `progress.yaml`；②“`live-04/05` 台账里 `| tail` 取的 rc 标签使 L-06 结论作废”——标签确不可当退出码证据，但 L-06 链的退出码来自 `l06-live/verify-l09.out` 与本轮 `verify-l11/l13.out`（重定向后取 `$?`，非管道尾），结论不受影响。
- 判据收紧（一律“写侧不许产出消费侧必拒的记录”）：scope 两清单必须与记录自身 targets 双向一致；runId / 组件名 / scope 条目必须是单一路径元素（此前组件名直接进 verify 脚本路径与证据目录名，可越出 `--output`）；成功记录必须至少带 plan+execute-report 证据（否则“逐证据重哈希”可被删数组绕过）；空 scope 不得汇总成 `overall=pass`/exit 0；只读 no-op 同样先通过与 add 一致的计划绑定子二进制校验再落记录。回归侧补齐真实写入器混合 pass、类型伪造、形状 guard、生产失败记录被拒、no-op 二进制绑定、组件 spec↔角色工作负载一致性（把 metrics 改回旧值该用例即红）以及经 `RunVerify` 的端到端配额不可重开（ledger 文件名与内容键均为 base run；第二个子 run 被 “already recorded as …” 拒且不再删 Pod）。
- 证据口径（实机与单测分开写，不再混列）：实机各命中 rc=1 的拒绝类别是 plan 当记录、torn 副本、篡改副本、越界 `--only`、观察记录申领 acceptance（旧轮 5 类），以及本轮新 enforcement 的 7 类伪造（改类型消费、改类型当 `--base-run`、删证据、scope 虚报、scope 漏报、runId 为路径、组件名为路径）；失败/取消/unknown 记录、base 字节变化、外集群 uid、身份缺项**只有单元测试覆盖**，不得记作实机通过。最终发布 live-13（树 `b155498b0d2c41d23789016381c53a598cce12cb4a719049363515334d283e1e` / kk `d1a2ca98270b98799fb73bb84aeedd7ac7e6db308f60656288066d8352888fd4`，节点端 SHA256SUMS 5/5）；同一棵树另发 live-11（kk `86b4a41b…`）与中间发布 live-12（树 `b31da279…` / kk `6ced10756437b6f96394ea5b7f6169104034d87715300230513fef569c4b4f62`）。同树不同 kk 这一事实本身即为裁决依据：**发布身份必须成对记录“源码树 + 二进制摘要”，单记树指纹不足以标识一次发布。**
...
- 单列未闭合（不在本条内扩大范围）：①`operation=add` 记录分支仍未在实机跑过（需删现有 valkey 或启用未选组件，越界），记 `live_not_run`，由生产写入器→真实消费者集成测试覆盖；②“无需人工即跨重启保持离线”仍 `not_verified`（重启授权已用尽）；③fluent-bit smoke 出现一次“失败后重跑即过”（marker 全命中但 `metadata.txt` 0 字节、报告无明细），疑与固定名 `ani-fluent-bit-verify-client` 复用有关，属 H 系列稳健性，另轮处理；④首装 smoke 遗留 `ani-installer-smoke/ani-smoke-backend`（Failed）是底座唯一 not-running Pod，属清理卫生项；⑤`cancelled`/`unknown` 仅存在于消费者词汇，执行器把中断一律记成 `failed`，本轮只写实限制，不编造生产者状态。
