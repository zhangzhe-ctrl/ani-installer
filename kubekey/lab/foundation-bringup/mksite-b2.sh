...
mkdir -p "$R/inputs"
umask 077
cat > "$R/inputs/site-b2-cluster.yaml" <<YAML
name: ani-lab
installerNode: node1
ssh:
  user: ubuntu
...
components:
  ...
  nats:
    enabled: false
YAML
stat -c '%a %n' "$R/inputs/site-b2-cluster.yaml"
sha256sum "$R/inputs/site-b2-cluster.yaml"
echo "B2_SITE_WRITTEN=$R/inputs/site-b2-cluster.yaml"
