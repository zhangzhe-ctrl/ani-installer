#!/usr/bin/env bash
# Fedora-side launcher: runs verifyb3.sh on .20 as ubuntu through a pty so the
...
rsh 'cat > /tmp/_ani_b3_verify.sh' < "$L/verifyb3.sh"
rsh 'cat > /tmp/_ani_b3_vrun.sh' <<'EOS'
#!/usr/bin/env bash
umask 077
( cat "$HOME/.ani_pw"; printf "\n" ) | script -qec "bash /tmp/_ani_b3_verify.sh" /dev/null
EOS
rsh 'chmod 700 /tmp/_ani_b3_verify.sh /tmp/_ani_b3_vrun.sh; bash /tmp/_ani_b3_vrun.sh'
